...
export const productListQueryOptions = (storeId: string, params?: ProductsParams) =>
  queryOptions({
    queryKey: productDefs.list.key(storeId, params),
    queryFn: () => get<ProductsResponse>(productDefs.list.url(storeId), { params }),
  });

export const productDetailQueryOptions = (storeId: string, productId: string) =>
  queryOptions({
    queryKey: productDefs.detail.key(storeId, productId),
    queryFn: () => get<Product>(productDefs.detail.url(storeId, productId)),
  });

export const createProductMutationOptions = (queryClient: QueryClient, storeId: string) =>
  mutationOptions({
    mutationKey: productDefs.create.key(storeId),
    mutationFn: (payload: CreateProductData) => post<Product>(productDefs.create.url(storeId), payload),
    onSuccess: invalidateProductAll(queryClient, storeId),
  });

export const updateProductMutationOptions = (queryClient: QueryClient, storeId: string) =>
  mutationOptions({
...
export const deleteProductMutationOptions = (queryClient: QueryClient, storeId: string) =>
  mutationOptions({
    mutationKey: productDefs.delete.key(storeId),
    mutationFn: (productId: string) => del<Product>(productDefs.delete.url(storeId, productId)),
    onSuccess: invalidateProductAll(queryClient, storeId),
  });

export const addProductImageMutationOptions = (queryClient: QueryClient, storeId: string) =>
  mutationOptions({
...
export const removeProductImageMutationOptions = (queryClient: QueryClient, storeId: string) =>
  mutationOptions({

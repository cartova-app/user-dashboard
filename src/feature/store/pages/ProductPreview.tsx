...
const ImageGalleryCard = ({ images }: { images: ProductImage[] }) => {
  ...
  // Thumbnails = all images except the active main one, capped at 3
  const thumbnails = images
    .map((img, idx) => ({ img, idx }))
    .filter(({ idx }) => idx !== activeIndex)
    .slice(0, 3);

  const extraCount = images.length - 1 - 3; // images beyond what thumbnails show
  ...
  return (
    <Card className="bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-2xl p-4 flex gap-4">
      {/* MAIN IMAGE */}
      <div className="flex-1 flex items-center justify-center bg-white dark:bg-[#111] rounded-xl min-h-[220px]">
      ...
      {hasThumbnails && (
        <div className="flex flex-col gap-2">
          {thumbnails.map(({ img, idx }, thumbPos) => {
            const isLastThumb = thumbPos === 2;
            const showOverlay = isLastThumb && extraCount > 0;
            ...
            return (
              ...
              >
                ...
                />
                {/* "+N more" overlay on last thumbnail when extras exist */}
                {showOverlay && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
            ...
            <Card className="bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-2xl p-6 flex flex-col justify-between">
              <div className="flex justify-between items-start">
                ...
                <Badge className="bg-green-100 text-green-600 rounded-full h-8 px-3">
                  Active
                </Badge>
              </div>
        ...
        <Card className="bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="bg-purple-100 p-2 rounded-lg">
              <Box className="text-purple-500 size-5" />
            </div>
            <div className="bg-gray-100 dark:bg-[#111] text-gray-500 dark:text-gray-400 text-sm px-3 py-1 rounded-lg flex items-center gap-1 cursor-pointer">
              All-time
                  <svg className="size-3" viewBox="0 0 20 20" fill="currentColor" role="img">
        ...
        <Card className="bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="bg-red-100 p-2 rounded-lg">
              <ShoppingBag className="text-red-500 size-5" />
            </div>
            <div className="bg-gray-100 dark:bg-[#111] text-gray-500 dark:text-gray-400 text-sm px-3 py-1 rounded-lg flex items-center gap-1 cursor-pointer">
              All-time
              <svg className="size-3" viewBox="0 0 20 20" fill="currentColor" role="img">

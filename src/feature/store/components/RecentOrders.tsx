import { BarChart3, Calendar, Edit3, Folder, Hash, Package } from 'lucide-react';
...
export default function RecentOrdersTable({ ordersData }: RecentOrdersTableProps) {
  ...
  const columns: DataTableColumn<OrderItem>[] = [
    {
      ...
      renderCell: ({ row }: { row: OrderItem }) => <span className="font-semibold text-foreground">#{row.id}</span>,
    },
    {
      field: 'productName',
  ...
  return (
    <div className="p-8 bg-card rounded-2xl border border-border shadow-sm mt-8">
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold tracking-tight text-card-foreground">Recent Orders</h2>

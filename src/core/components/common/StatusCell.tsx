...
const StatusCell = ({ status }: StatusCellProps) => {
  ...
  const currentStatus = statusList.find((item) => item.value === normalizedStatus);
  ...
  if (!currentStatus) {
    return (
      <div className="flex justify-center items-center px-2.5 py-1 gap-1.5 rounded-xl bg-gray-200 dark:bg-gray-800">
        ...
        <p className="text-gray-700 dark:text-gray-300 font-satoshi text-xs font-normal leading-4">{status}</p>
      </div>
    );
  }

  return (
    <div className={cn('flex justify-center items-center px-2.5 py-1 gap-1.5 rounded-xl', currentStatus.className)}>
      <div className={cn('w-2 h-2 rounded-full animate-pulse', currentStatus.dotClassName)}></div>
      <p className="font-satoshi text-xs font-normal leading-4 text-inherit">{currentStatus?.name}</p>
    </div>
  );
};

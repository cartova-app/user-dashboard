                  ...
                  <SidebarMenuItem key={item.title}>
                    ...
                    >
                      <Link to={item.url} className="flex items-center gap-3 w-full">
                        {/* Active indicator */}
                        {isActive && (
                          <div className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-1 rounded-r-full bg-primary dark:bg-primary group-data-[collapsible=icon]:hidden" />
                        )}

                        {/* Icon */}
                        <item.icon
                          className={`size-5 shrink-0 transition-colors duration-200 ${isActive ? 'text-primary dark:text-primary' : 'text-muted-foreground/80 group-hover:text-sidebar-accent-foreground'}`}
                        />
